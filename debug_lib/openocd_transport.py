...
class OpenOCDTransport(DebugTransport):
    ...
    def connect(self) -> None:
        ...
        for _ in range(30):
            try:
                ...
                last_error = e
                time.sleep(0.5)
                # Drain any banner OpenOCD sends on connect
                # is probably unnecessary and potentially dangerous.
                # OpenOCD TCL server usually does NOT send a meaningful startup banner like telnet mode.
                # Your drain code may accidentally consume the first real command response.
                # I would remove it entirely initially
                #self._drain_banner()
            #except (ConnectionRefusedError, OSError) as e:
    ...
    def read(self, addr: int) -> int:
        """
        DMI read using raw JTAG scans via OpenOCD TCL.
        
        DMI DR is 41 bits: {addr[6:0], data[31:0], op[1:0]}
        op=1 means read, op=0 means NOP (to capture response)
        """
        tap = "riscv.cpu"
        # Select DMI register (IR = 0x11 = 17 decimal, 5 bits)
        self._send_tcl(f"irscan {tap} 0x11")
        
        # Send read request: addr in [40:34], data=0 in [33:2], op=1 in [1:0]
        dr_val = (addr << 34) | (0 << 2) | 1  # op=1 (read)
        self._send_tcl(f"drscan {tap} 41 0x{dr_val:011x}")
        
        # Send NOP to capture the response
        dr_nop = 0  # op=0 (nop)
        resp = self._send_tcl(f"drscan {tap} 41 0x{dr_nop:011x}")
        
        # Parse the response: data is bits [33:2]
        resp_val = self._parse_int(resp, context=f"read addr=0x{addr:02x}")
        data = (resp_val >> 2) & 0xFFFFFFFF
        op_resp = resp_val & 0x3
        log.debug("[OpenOCDTransport] read addr=0x%02x → data=0x%08x op=%d (raw=0x%x)",
                  addr, data, op_resp, resp_val)
        return data
