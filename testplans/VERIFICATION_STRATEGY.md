...
- **Keeping the two models the same model (2026-09-17, #75).** `dm_ref_model.sv` and `predictor.py` now cover the same scope, and they are held together by execution rather than by inspection. With `+DM_MODEL_TRACE` the SV model records every input and prediction; `mk/model_crosscheck.py` replays that through the Python model and compares them at every read, `dmstatus` both before and after the hart-signal sync. The full regression agrees on every read, and three deliberately broken Python rules are each caught. Method and numbers: `testplans/results/model_crosscheck_2026-09-17.md`. The Python model is still not run *live* alongside the checker; it is checked against the same stream afterwards.
