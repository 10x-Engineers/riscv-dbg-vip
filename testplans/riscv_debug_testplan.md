...
| TC-HOR-002 | `setresethaltreq` causes halt-on-reset | Write `dmcontrol.setresethaltreq=1` for selected hart, then assert/deassert reset (any cause) | Hart enters Debug Mode immediately on reset deassertion, regardless of reset cause | 4 (`setresethaltreq=1`, `in_reset→halted` transition, `ndmreset_x_reset_haltreq=assert_rhr1`/`deassert_rhr1`) | P1 |
| TC-HOR-003 | `clrresethaltreq` clears the request | After `setresethaltreq=1`, write `clrresethaltreq=1` before the next reset | Hart does *not* halt on the next reset deassertion | 2 (`clrresethaltreq=1`, `ndmreset_x_reset_haltreq=deassert_rhr0` — this bin is shared proof that `clrresethaltreq` worked) | P1 |
...
| `dmstatus.stickyunavail=1` | `stickyunavail` is `Preset` (#3.14.1) and both project DUTs are configured to report 0. With it 0, `unavail` cannot become sticky, so `allunavail`/`anyunavail=1` and the corresponding `all_any.unavail` bins are unreachable *on these DUTs specifically* — see the Uncertain bucket below for whether that's worth revisiting on a DUT with `stickyunavail=1`. |
...
| `dmcontrol.setkeepalive`/`clrkeepalive` (both values) | These fields exist in the current ratified `riscv-debug-spec` register XML but this project's testplan explicitly targets **v1.0.0-rc3**. Whether `keepalive` was present in rc3 or added later has not been confirmed — do not write TC-IDs against it until that's resolved one way or the other. `registers.py`/`predictor.py` do not model these fields at all yet, so there is no golden-model behavior to check against either. |
| `dmstatus.stickyunavail=1` and its downstream `unavail`-sticky bins | Both project DUTs (CVA6, Ibex) report `stickyunavail=0` today. Revisit if a future DUT configures it differently — the model already supports it (`DMPredictor`'s `unavail_sticky` field), only stimulus and a capable DUT are missing. |
| `dmcontrol.ackunavail` (both values), and the `unavail`-state `dmstatus` bins it depends on | The model implements the conditional-clear semantics (`ackunavail` clears `unavail` only for harts that are *currently available*, per #3.14.2) and `HartState.available` exists to drive it, but no sequence yet drives a hart through an actual unavailable→available transition to exercise it. Needs the hart-availability mechanism itself modeled first (a later slice — likely paired with multi-hart work). |
