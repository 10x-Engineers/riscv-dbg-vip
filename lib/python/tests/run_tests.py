...
from rv_dbg_python_api.transport import TransportError
from rv_dbg_python_api.riscv_dm  import RISCVDebug, DebugError, DMI
from rv_dbg_python_api.session   import DebugSession, StepResult

# ── MockTransport (copy from test file, no external deps) ─────────────────────
from rv_dbg_python_api.transport import DebugTransport
...
def t_transport_swap():
    from py_seq_lib.halt_sequence import build_halt_sequence
    t = MockTransport(); t.connect()
    dm = RISCVDebug(t)
...
def t_halt_sequence_resume_after():
    from py_seq_lib.halt_sequence import build_halt_sequence
    t = MockTransport(); t.connect()
    dm = RISCVDebug(t)
    s = build_halt_sequence(dm, mode="batch", resume_after=True)
    s.run()
    assert_true(s.all_passed)
    assert_false(t.halted)
test("halt_sequence can resume after inspection", t_halt_sequence_resume_after)
