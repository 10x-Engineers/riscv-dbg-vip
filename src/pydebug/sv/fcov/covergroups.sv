...
`endif
    ...
    // ══════════════════════════════════════════════════════════════════════════
    // Sdext coverage -- merged in from dm_spec_coverage.sv.
    //
    // The covergroups above sample DMI-visible DM registers. These sample the
    // HART: dcsr, dpc, privilege and which instruction a step stepped over all
    // live in the core, reachable over DMI only through an abstract command.
    // That gap is why the wfi single-step deadlock could not appear as a
    // coverage hole -- no bin represented "a step over a stalling instruction".
    //
    // One class rather than two, because two subscribers on the same analysis
    // port sampling overlapping state is a merge problem nobody wants: the
    // per-run numbers cannot be added, and which file owns a given behaviour
    // stops being obvious.
    // ══════════════════════════════════════════════════════════════════════════
    ...
    function new(string name, uvm_component parent);
        ...
        cg_data0_access    = new();
        cg_trigger         = new();
        // Sdext covergroups. Embedded covergroups can only be constructed in
        // the class's own new(), same constraint as the ones above.
        cg_debug_entry   = new();
    ...
    function void build_phase(uvm_phase phase);
        ...
        // Sdext coverage samples the hart, not the DMI bus. Without these the
        // Sdext covergroups simply never sample -- warn rather than fail, so a
        // SoC that publishes no backdoor still runs the DM-register coverage.
        if (!uvm_config_db #(virtual dbg_hart_backdoor_if)::get(
                this, "", "hart_backdoor_vif", hart_vif))
            `uvm_warning("DBG_COV", "no hart backdoor -- Sdext coverage will not sample")
        if (!uvm_config_db #(virtual dbg_dm_backdoor_if)::get(
                this, "", "dm_backdoor_vif", dm_vif))
            `uvm_warning("DBG_COV", "no DM backdoor -- cmderr/sbcs coverage degraded")
    endfunction
    ...
    function void write(jtag_txn_c t);
        ...
        // ── Sdext sampling ───────────────────────────────────────────────────
        // Same transaction, different question: the decode above asks what the
        // debugger wrote, this asks what state the DM is in as a result. Read
