"""
tests/test_riscv_dm.py — Unit tests for the pydebug library.
...
import pytest
from pydebug.api.transport import DebugTransport, TransportError
from pydebug.api.riscv_dm  import RISCVDebug, DebugError, DMI
from pydebug.api.session   import DebugSession, StepResult
...
class MockTransport(DebugTransport):
    ...
    def read(self, addr: int) -> int:
        ...
        if addr == DMI.ABSTRACTCS:
            return 0x0100_0000   # busy=0 (bit12=0), cmderr=0, progbufsize=1 (bits[28:24])
        if addr == DMI.DATA0:
            return self.regfile.get(DMI.DATA0, 0xDEAD_BEEF)
        if addr == DMI.SBCS:
            return 0x2004_0000   # sbbusy=0 (bit21=0), sbversion=1 (bits[31:29]), sbaccess32=1 (bit17)
        if addr == DMI.SBDATA0:
...
def test_halt_sequence_can_resume_after_inspection():
    ...
    session = build_halt_sequence(dm, mode="batch", resume_after=True)
    session.run()
...
def test_sv_kit_files_exclude_templates():
    ...
    `pydebug sources --sv` would otherwise pull in every SoC's top module
    at once."""
    from pydebug.bridge_utils import get_sv_kit_files, get_template_dir
    files = get_sv_kit_files()
